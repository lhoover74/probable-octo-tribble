...
export default async function CloudflareTowRequestsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-semibold text-white">Cloudflare Tow Requests</h1>
        <p className="mt-2 text-sm text-slate-400">Open tow queue pulled from the D1 database binding.</p>
      </div>
      ...
      <SectionCard title="Active Cloudflare queue" description="D1-backed tow queue entries waiting for dispatch or arrival.">
        <div className="space-y-3">
          {active.map((vehicle) => (
            <Link key={vehicle.id} href={`/cf/vehicles/${vehicle.id}`} className="block rounded-2xl border border-slate-800 bg-slate-950/40 p-4 transition hover:border-slate-700">
