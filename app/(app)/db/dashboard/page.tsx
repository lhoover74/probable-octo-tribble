...
export default function DbDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-semibold text-white">DB Dashboard</h1>
        <p className="mt-2 text-sm text-slate-400">Operational summary reading from SQLite-backed vehicle records.</p>
      </div>

      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        <MetricCard label="Vehicles In DB" value={vehicles.length} icon={Car} />
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <SectionCard title="Counts by property" description="Current database case volume by managed location.">
        ...
        <SectionCard title="Status mix" description="Open and closed database case distribution.">
      ...
      <SectionCard title="Recent activity" description="Latest database-backed record updates.">
        <div className="space-y-4">
          {recentActivity.map((entry) => (
            <Link key={entry.id} href={`/db/vehicles/${entry.vehicleId}`} className="block rounded-2xl border border-slate-800 bg-slate-950/40 p-4 transition hover:border-slate-700">
