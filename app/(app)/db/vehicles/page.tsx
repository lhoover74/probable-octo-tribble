...
export default function DbVehiclesPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-3xl font-semibold text-white">DB Vehicle Records</h1>
          <p className="mt-2 text-sm text-slate-400">SQLite-backed records with real database persistence.</p>
        </div>
        <Link href="/db/vehicles/new" className="rounded-2xl bg-emerald-500 px-4 py-3 text-sm font-medium text-slate-950">
          Add DB Record
        </Link>
      </div>
              ...
              {vehicles.map((vehicle) => (
                <tr key={vehicle.id} className="border-t border-slate-800">
                  <td className="px-4 py-4">
                    <Link href={`/db/vehicles/${vehicle.id}`} className="font-medium text-white hover:text-emerald-300">
                      {vehicle.plate}
                    </Link>
