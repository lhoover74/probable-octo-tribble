...
import { createSqliteVehicleAction } from "@/lib/sqlite-actions";
import { properties, towReasons, statuses } from "@/lib/mock-data";

export default function DbNewVehiclePage() {
  const property = properties[0];

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-semibold text-white">DB Add Vehicle</h1>
        <p className="mt-2 text-sm text-slate-400">This form saves directly into the SQLite database file.</p>
      </div>

      <SectionCard title="Create database-backed record" description="Submitting writes to SQLite and redirects to the saved record.">
        <form action={createSqliteVehicleAction} className="grid gap-4 md:grid-cols-2">
          {[
